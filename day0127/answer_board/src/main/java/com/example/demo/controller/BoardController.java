...
public class BoardController {
	...
	public void insertForm(Model model,@RequestParam(value = "no",defaultValue = "0") int no) {
		//답글 달 때만 no를 줘야해서 -> @RequestParam(value = "no",defaultValue = "0") int no
	...
	public ModelAndView insertSubmit(BoardVO b,HttpServletRequest request) {
		...
		//만약에 답글이라면
		int pno = b.getNo();			//새로 등록하는 글번호 아니고 부모글번호
		if(pno != 0) {
		...
		int re = dao.insert(b);	//insert성공하고 
		if(re == 1) {
			if(b.getFsize() != 0) {		//파일 업로드 하였다면
				String path = request.getRealPath("upload");		//실경로 읽어오고
